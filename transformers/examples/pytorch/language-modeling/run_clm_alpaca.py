...
def main():
    ...
    def preprocess_function(examples):
        with CaptureLogger(tok_logger) as cl:
            ...
            result = tokenizer(examples[text_column_name], max_length=block_size, padding=padding,
                               truncation=True, return_length=True)
            labels = copy.deepcopy(result["input_ids"])
            if "prefix" in column_names:        # Do not compute loss for prompt inputs
                prefix_tokenized = tokenizer(examples["prefix"], max_length=block_size, padding=padding,
                                   truncation=True, return_length=True)
                for label, prefix_len in zip(labels, prefix_tokenized["length"]):
